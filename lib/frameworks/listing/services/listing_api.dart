...
class ListingAPI {
  ...
  ListingAPI(this.url, this.consumerKey, this.consumerSecret) {
  ...
  Future<dynamic> getAsync(String endPoint, {int version = 2}) async {
    try {
      ...
      printLog(
          "[listing_api][${DateTime.now().toString().split(' ').last}] getAsync START [endPoint:$endPoint] url:$url");
      ...
      printLog(
          "[listing_api][${DateTime.now().toString().split(' ').last}] getAsync END [endPoint:$endPoint] url:$endPoint");
  ...
  Future<dynamic> postAsync(String endPoint, Map data,
    ...
    printLog(
        "[listing_api][${DateTime.now().toString().split(' ').last}] postAsync START [endPoint:$endPoint] url:$url");
    ...
    printLog(
        "[listing_api][${DateTime.now().toString().split(' ').last}] postAsync END [endPoint:$endPoint]");
    return dataResponse;
  }
